...
(медиана 1,25). Рекуррентная механика *закрыла бы* диапазон при измеренном k ≥ 6,31;
k в музыке UNMEASURED (emp2; Twitch k=12 не closer). Красная команда отозвала одно
число v0.2 и заменила три точечных допущения диапазонами.
...
дропы (Tonify — attention economy на рельсах Telegram/TON). Вопрос работы: на сколько порядков различаются миры
...
T1 87% треков <1000 стримов/год (Luminate 86,2%; стилизация на мир артистов) → получено 87,0% ✓
...
- Кеф соседних индустрий (Twitch/Patreon k=12, Tencent gifting) диапазон **не**
  закрывает; слот измерения — emp2, статус UNMEASURED.
...
## 7. Что построено (ТЗ: CLAUDE_CODE_HANDOFF.md)
- SIM 2 — социальный граф Telegram: BA+клики, complex contagion, фазовая диаграмма K, посев в хабы. [x]
- SIM 3 — анти-кладбище: закон «выплаты ≤ приток» против эмиссии, калибровка на Hamster 300M→12M (AInvest / CryptoPotato сент. 2024, не Caladan). [x]
- SIM 4 — полный двудольный user×artist (вместо Monte-Carlo доли кошелька). [x]
- SIM 5 — склейка: каскад → σ → k/чек (emp2) → казна. [x]
- emp2 — кеф платежей Telegram ≥200 строк. Протокол [x]; измерение UNMEASURED.
...
Итог как решение, не как теорема: [Что делать](WHAT_NEXT.ru.md) — приемлемая реформа формулы и отказ от роялти за play (Tonify → Unify).

## 8. Ограничения
Синтетический мир: форма хвоста между якорями — конструкция; UC в sim1 — аппроксимация кошелька (полный двудольный — sim4);
кеф в музыке UNMEASURED (emp2; Twitch не closer); чек и доля суперфанов — диапазоны; Tencent-бенчмарк —
...
fig19 сетка прохода · fig20 склейка σ*
...
---
## CHANGELOG v1.2 (август 2026) — склейка, слот кефа, сетка прохода, гигиена

sim5 склеивает sim2 → σ → k/чек (emp2) → казну sim3; имя sim4 уже занято матрицей
прослушиваний. emp2 — fail-closed слот кефа Telegram: без ≥200 строк UNMEASURED;
Twitch k=12 отозван как closer диапазона 0,38–6,31 (ступень Twitch-механики fig5
остаётся сравнением take rate). v07 + fig19: проход ρ = 6,772 / 10,6 / 20% при
фикс. инди $4,43/1k. RESULTS больше не ведёт с отозванного 0,42. Секция Unify
снята (вне скоупа). sim2/3/4/5 отмечены построенными. Атрибуция Caladan снята
с подписей/SPEC (AInvest / CryptoPotato).
