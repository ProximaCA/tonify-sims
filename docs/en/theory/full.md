...
\mathrm{PR}_i=0`$ ($`A_i=0`$ implies $`P_i=0`$). *User-centric*:
...
**Theorem 1 1** (Identity). *For any artist $`i`$ with $`P_i>0`$, under
equal wallets,
``` math
...
*Proof.* $`\mathrm{UC}_i/\mathrm{PR}_i
=\bigl(W\sum_u p_{iu}/P_u\bigr)\big/\bigl(UW P_i/T\bigr)
=(T/U)\sum_u (p_{iu}/P_i)(1/P_u)=\bar{P}\,\mathbb{E}_{w^i}[1/P_u]`$. ◻
...
**Corollary (a) 1** (Coincidence; the naive converse is false).
...
**Corollary (c) 1** (Crossover). *$`\mathrm{UC}_i>\mathrm{PR}_i`$ iff
...
**Corollary (d) 1** (Dispersion premium). *Fix $`m_i=\bar{P}`$. Then
...
**Lemma 2 1** (Two-part invariance). **(2a)* For any pool operator $`F`$
...
**Theorem 4 1** (Full blockade). *(i) $`Y_A`$ never first-order
...
\bigr)^A`$ with $`\varphi(t)=\mathbb{E}e^{-t\tau G}<1`$ — exponential
