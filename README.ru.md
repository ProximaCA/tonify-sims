...
# tonify-sims: шесть симуляций прямой музыкальной экономики
...
полных диапазонах, не в краях). В этом репозитории — шесть
детерминированных симуляций прямой музыкальной экономики на рельсах
...
измеренных якорях (87% треков ниже 1 000 стримов/год — Luminate, стилизация на мир артистов; 2,6%
...
см. *Отозвано и ограничено*). Кеф соседних индустрий (Twitch/Patreon k=12)
диапазон **не** закрывает — k в музыке UNMEASURED ([emp2](emp2/README.md)).
Рекуррентный кеф 12 на TON-рельсе ронял бы MVA кассы до 900 *если бы* этот k
был измерен. Ступень Twitch-механики (2 353) считана на собственной фикс-цене
подписки Twitch $5; на том же среднем чеке $6,89, что и direct-строки, она
дала бы 1 709 — преимущество прямой экономики над Twitch-механикой в take
rate (5% против сплита 50%), а не в рельсе ([CRITIC](paper/CRITIC.ru.md) §1;
...
![fig20_glue_sigma](figures/ru/fig20_glue_sigma.png)
...
([sim5](sim5/SPEC.md); [emp2](emp2/README.md)).

![fig19_passthrough](figures/ru/fig19_passthrough.png)
...
качественный тезис нет ([sim1/v07_passthrough.py](sim1/v07_passthrough.py)).
...
![fig21_floor_zeros](figures/ru/fig21_floor_zeros.png)
...
independent: $B$ и $D$ не проходят через $\rho$ ([sim6](sim6/SPEC.md)).

![fig22_floor_mva](figures/ru/fig22_floor_mva.png)
...
```
python3 run_all.py    # sim1–6 + слот emp2 + фигуры EN/RU -> ./figures + ./figures/ru
```
...
```
tonify-sims/
├── run_all.py        # sim1–6 + слот emp2, фигуры
├── README.md         # EN, первичный · README.ru.md — этот файл
├── SOURCES.md        # параметр -> значение -> источник -> vault
├── paper/            # PAPER / RESULTS / CRITIC / THEORY / WHAT_NEXT (policy)
├── sim1/             # касса против котла (+ v07 сетка прохода)
├── sim2/             # complex contagion на синтетическом Telegram-графе
├── sim3/             # закон казны против эмиссии
├── sim4/             # двудольная матрица прослушиваний
├── sim5/             # склейка: каскад → σ → k/чек → казна
├── sim6/             # Direct + guaranteed discovery floor
├── emp1/             # сопряжение γ на Last.fm (внешние ~4 ГБ)
├── emp2/             # кеф платежей Telegram (fail-closed; UNMEASURED)
├── docs/             # сайт GitBook (генерируется)
├── figures/          # fig1–fig22 (EN) · figures/ru/ — русские
└── LICENSE           # MIT
```
...
- **sim5 — калькулятор на замороженных числах sim2, не новый граф.** Один
  трек, нет конкурирующих каскадов, нет забывания (потолок sim2). σ — полоса,
  k — UNMEASURED (emp2). Масштаб 50k → 1M — линейная плотность, не пересчёт
  графа на 1M. Кросс-сим связка теперь в скоупе как арифметика, не как общая
  динамика.
