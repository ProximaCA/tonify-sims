...
месяц на подписанном pro-rata стриминговом контракте, — или 3 204 напрямую
платящих суперфана при четырёх донатах в год. В этом репозитории — три
...
формуле. Формула двигает жизнеспособность артиста в ×1,34. Контракт — в ×14,8.
Точно: в полной матрице {правило × контракт} минимальной жизнеспособной
аудитории смена правила дележа (pro-rata → user-centric) сдвигает MVA артиста
в ×1,34, а смена контракта (signed → independent) — в ×14,8 (fig7;
[PAPER](paper/PAPER.ru.md), Addendum v0.5). Каждое утверждение ниже несёт либо
...
Происхождение каждого параметра — одна таблица: [SOURCES.ru.md](SOURCES.ru.md).
...
модель: [paper/PAPER.ru.md](paper/PAPER.ru.md); числа:
[paper/RESULTS.ru.md](paper/RESULTS.ru.md); отозванное:
[paper/CRITIC.ru.md](paper/CRITIC.ru.md); источник каждого параметра:
[SOURCES.ru.md](SOURCES.ru.md).
...
**Находка 1 — контракт весит больше правила.** По полной MVA-матрице
{правило × контракт} смена правила дележа (pro-rata → user-centric) двигает
минимальную жизнеспособную аудиторию в ×1,34, смена контракта
(signed → independent) — в ×14,8: лучшая формула World A не переживает
лейбловый проход 6,8% — user-centric signed требует 140 463 слушателя против
12 771 у pro-rata independent. Лестница $100/мес: 188 590 (pro-rata signed) →
12 771 (pro-rata independent) → 3 204 (касса при кефе 4
доната/суперфан/год); контракт signed-360 масштабирует долю артиста в ×0,70
...
каждая линия сетки — в ×10 меньше нужных слушателей. Смена правила дележа
двигает MVA в ~×1,3; смена механизма — на 1–2 порядка (188 590 → 900 при
рекуррентном кефе 12).*
...
6,31, а рекуррентный кеф 12 на TON-рельсе роняет MVA кассы до 900
...
медианный донат $6,9 против того же порога $13. Из $1 на TON-рельсе до
...
протокол и валидация: [sim2/README.md](sim2/README.md),
...
изолируется при B ≥ 50 (эксперимент A; [sim2/README](sim2/README.md) §4.1
v1.3).
...
48,3%, посев из одного чата). Анимация выше — превью; полный файл:
[figures/fig10_cascade.gif](figures/fig10_cascade.gif) (2,6 МБ, 15 кадров,
раунд 0 → 14).*
...
фальсификаторы: [sim3/README.md](sim3/README.md),
...
- **Детерминизм:** seed=42 везде; повторный запуск даёт байт-в-байт идентичный
  stdout и байт-в-байт идентичные фигуры.
- **Валидация до выводов:** каждая симуляция печатает свои валидационные
  мишени (мишень → получено → PASS/FAIL) *до* результатов; FAIL блокирует
  выводы.
...
- **По отдельности:** `python3 sim1/tonify_cash_sim.py` (затем
  `sim1/v04_full.py`, `sim1/v05_matrix.py`), `python3 sim2/tonify_graph_sim.py`
  (~36 с), `python3 sim3/sim3_anti_graveyard.py` (~1 с).
- **Происхождение:** каждый параметр модели → использованное значение →
  источник → vault-заметка в [SOURCES.ru.md](SOURCES.ru.md).

## Структура репозитория

```
tonify-sim/
├── run_all.py        # одна команда: sim1 + sim2 + sim3, фигуры fig1-fig13
...
├── sim1/             # касса против котла: tonify_cash_sim.py, v04_full.py,
│                     #   v05_matrix.py (матрица {правило x контракт})
...
[paper/CRITIC.ru.md](paper/CRITIC.ru.md); sim2/sim3 — CHANGELOG-блоки в
[sim2/SPEC.md](sim2/SPEC.md) и [sim3/SPEC.md](sim3/SPEC.md).
...
(проверено красной командой на независимых батчах сидов). Отозванные числа
перечислены в [paper/CRITIC.ru.md](paper/CRITIC.ru.md) (sim1) и
CHANGELOG-блоках спек ([sim2/SPEC.md](sim2/SPEC.md),
[sim3/SPEC.md](sim3/SPEC.md)).
